            ...
            if (Math.random() < 0.03) {
                ...
                for (let k = 0; k < columnLength; k++) {
                    if (j+k >= 0 && j+k < l_screenSpaceY) {
                        ...
                        l_a_screenSpace[j+k][i].debugElement.classList.add("cl_debugMask_bkg");
                    }
                }
            }
        }
...
function dropStep() {
    for (let i = 0; i < countPerDropType; i++) {
        /* First decide if the drop will die. */
        ...
        if (a_igniterDrops[i].x >= 0 && a_igniterDrops[i].x < screenSpaceX && a_igniterDrops[i].y >= 0 && a_igniterDrops[i].y < screenSpaceY) {
            a_screenSpace[a_igniterDrops[i].x][a_igniterDrops[i].y].touched = 1;
        }
    }
}
...
function dropFluctuation() {
    /* Decide if it will happen in screen space or not. */
...
function step(l_a_screenSpace) {
    dropStep();
    dropFluctuation();

    for (let j = 0; j < l_a_screenSpace.length; j++) {
        for (let i = 0; i < l_a_screenSpace[j].length; i++) {
            if (!l_a_screenSpace[j][i].blind && !l_a_screenSpace[j][i].static) {
                changeCharacter(l_a_screenSpace[j][i].a_element[activePage]);
            }
        }
    }
...
const animation = setInterval(() => step(a_screenSpace), 250);
